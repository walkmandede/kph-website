            ...
            horizontalBuilder: (xHorizontal) {
              return Column(
                ...
                children: [
                  ...
                  ...MyProjectRepo.myProjectsData.map((each) {
                    return SizedBox(
                      width: double.infinity,
                      child: Card(
                        margin: EdgeInsets.only(
                          bottom: AppConstants.basePaddingFactorS.ofScreenHeight(context: context)
                        ),
                        color: Theme.of(context).scaffoldBackgroundColor.withValues(alpha: 0.8),
                        shape: RoundedRectangleBorder(
                          borderRadius: BorderRadius.circular(AppConstants.baseBorderRadius),
                          side: BorderSide(
                            color: Theme.of(context).secondaryHeaderColor.withValues(alpha: 1),
                            width: 1.5
                          )
                        ),
                        child: Padding(
                          padding: EdgeInsets.symmetric(
                            horizontal: AppConstants.basePaddingFactorS.ofScreenWidth(context: context),
                            vertical: AppConstants.basePaddingFactorS.ofScreenWidth(context: context),
                          ),
                          child: Column(
                            crossAxisAlignment: xHorizontal?CrossAxisAlignment.start:CrossAxisAlignment.center,
                            spacing: 10,
                            children: [
                              ...
                              if(each.onlineImages.isNotEmpty)AppConstants.basePaddingFactorS.ofScreenHeight(context: context).heightBox(),
                              if(each.onlineImages.isNotEmpty)SizedBox(
                                ...
                                child: SingleChildScrollView(
                                  ...
                                  child: Row(
                                    spacing: 10,
                                    children: [
                                      ...each.onlineImages.map((eachImage) {
                                        return ClipRRect(
                                          borderRadius: BorderRadius.circular(AppConstants.baseBorderRadius),
                                          child: AspectRatio(
                                            aspectRatio: 9/18,
                              ...
                              if(each.metas.isNotEmpty)AppConstants.basePaddingFactorS.ofScreenHeight(context: context).heightBox(),
                              if(each.metas.isNotEmpty)Column(
                                spacing: 10,
                                children: [
                                  ...each.metas.map((eachMeta) {
                                    return TextField(
                                      controller: TextEditingController(text: eachMeta.data),
                                      readOnly: true,
                                      style: AppConstants.getFont1.copyWith(
                                          fontSize: AppConstants.baseFontSizeM,
                                          fontWeight: FontWeight.bold,
                                      ...
                                      decoration: InputDecoration(
                                          ...
                                          prefixIcon: IconButton(onPressed: () {
                                            try{
                                              launchUrl(Uri.parse(eachMeta.data));
                                            }
                                            catch(_){}
                                          }, icon: Icon(Icons.open_in_new_rounded,color: Theme.of(context).colorScheme.secondary,)),
                                          labelStyle: AppConstants.getFont1.copyWith(
                                              fontSize: AppConstants.baseFontSizeM,
                                              fontWeight: FontWeight.bold,
                                              color: Theme.of(context).hintColor
                                          )
